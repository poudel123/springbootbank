...
public class CustomerRepositoryImpl implements CustomerRepository {
	...
	public Customer authenticate(Customer customer) throws DataAccessException {
		try {
			return jdbcTemplate.queryForObject("select * from customerdata where customerId=? and password=?",
	...
	public Customer updateProfile(Customer customer) throws DataAccessException{
		try{
			jdbcTemplate.update("UPDATE customerdata SET address=?,email=?  WHERE customerId=?",
		
				new Object[] { customer.getAddress(), customer.getEmail(), customer.getCustomerId() });
		return customer;
	} catch(DataAccessException e) {
	...
	public boolean updatePassword(Customer customer, String oldPassword, String newPassword)
		...
		try {
				int count = jdbcTemplate.update("UPDATE customerdata SET password=?  WHERE customerId=? and password=?",
	...
	private class CustomerRowMapper implements RowMapper<Customer> {
		...
		public Customer mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			bankAccount = jdbcTemplate.queryForObject("select * from bankdata where accountid=?",
					new Object[] { rs.getInt(7) }, new BankAccountRowMapper());
